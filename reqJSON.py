...
# pk값은 auto incresment로 그냥 주고 등록일자로 검색하게 하는게 좋을라나? (구분명은 어떨까요? - 호용)

# 조회할 날짜 변수 나중에 flask로 웹에서 변수 받아서 넣는것으로 바꿀예정 리턴값으로 데이터프레임 받아옴

# flask에서 비동기로 1. 날짜별  (2. 구분별 3. 성별) 등 한두가지 요소 더추가해서 탭 클릭시 해당 df 테이블로 불러오기?
# 날짜 - line / 구분명 - bar matplotlib.pyplot 써보기? 
...
def getCovidinfo(date):
    ...
    return df.sort_values(by='구분명').set_index("등록일자")
